...
""" Use this script to convert the annotation of the REAL-Colon dataset from the VOC format to the COCO format.
    ...
    orig: if negative_ratio 0.25 then 25% of all negaitve frames are selected
    now: if negative_ratio 0.25 then 25% negatives from total positives are selected
...
def convert_video_list(base_dataset_folder, video_list, annotation_list, frames_output_folder, json_output_file,
    ...
    for video_idx, (curr_video_folder, curr_ann_folder) in enumerate(zip(video_list, annotation_list)):
        ...
        # ADAPTION: Select the negative samples
        num_pos = len(selected_frames_w_box_indexes)
        to_keep = int(negative_ratio * num_pos)
        if negative_ratio <= 0:
            to_keep = len(frames_nobox_indexes)
        else:
            to_keep = int(negative_ratio * num_pos)
        to_keep = min(to_keep, len(frames_nobox_indexes))
        selected_frames = sorted(
            list(selected_frames_w_box_indexes) +
            random.sample(frames_nobox_indexes, to_keep)
        )
        xml_to_be_used = [all_xmls[y] for y in selected_frames]

        print(
            f"Selecting {to_keep} negative frames (ratio={negative_ratio} of positives = {num_pos}) "
            f"from {len(frames_nobox_indexes)} available negatives."
        )
        ...
        for c_xml in xml_to_be_used:
            ...
            data["images"].append({'license': 1, 'file_name': c_data['img_name'], 'height': c_data['img_shape'][0],
                                   'width': c_data['img_shape'][1], 'id': image_uniq_id_cnt})
            ...
            for cbox in c_data['boxes']:
                ...
                data['annotations'].append({'segmentation': [[l, t, r, t, r, b, l, b]], 'area': area,
                                            'iscrowd': 0, 'image_id': image_uniq_id_cnt, 'unique_id': cbox['unique_id'],
            ...
            os.symlink(
                os.path.relpath(os.path.join(base_dataset_folder, curr_video_folder, c_data['img_name']),
...
if __name__ == "__main__":
    # Parameters
    base_dataset_folder = "/data/local/aschwab/data/realColon"  # Path to the folder of the original REAL-COLON dataset (update with proper value)
    num_positives_per_lesions = -1 # Number of frames with boxes for each polyp to be included in the output dataset
    negative_ratio = 1 # Ratio of images without boxes for each video to be included in the output dataset [0,1]. -1 means keep all negatives
    ...
    output_folder = f"/data/local/aschwab/data/real_colon_allPos_fraction1ofNeg_onlyPatient" # Output folder for the converted dataset
    ...
    convert_video_list(base_dataset_folder, video_list_train, annotation_list_train, train_images_folder,
                       json_output_file_train, negative_ratio=negative_ratio,
    ...
    convert_video_list(base_dataset_folder, video_list_validation, annotation_list_validation, validation_images_folder,
                       json_output_file_validation, negative_ratio=negative_ratio,
                       num_positives_per_lesions=num_positives_per_lesions)
