...
# Reuses: ~/master-thesis/tools/event_eval/step2_event_froc.py  (logic unchanged)
#
# Usage:
#   bash step3_event_froc_val.sh \
#     --run y11m_224_s42_b512_baseM \
#     --data /data/local/aschwab/data/realColon_224x224/data.yaml \
#     [--imgsz 224] \
#     [--conf-sweep "0.05,0.10,0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.90"] \
#     [--link-iou 0.30] [--max-gap 3] [--min-len 2] [--match-iou 0.30] [--tiny-area-px 64] \
#     [--px-bins "0,16,32,1e9"] \
#     [-l|--labels-val /path/to/labels/val]
#
set -euo pipefail
...
if [[ -z "$LABELS_VAL" ]]; then
  ...
  if [[ -n "$DS_BASE" && -d "$DS_BASE/labels/val" ]]; then
    ...
    if [[ -f "$DATA" ]]; then
      VAL_IMG="$(awk '/^[[:space:]]*val:/ {print $2}' "$DATA" | tr -d "'\"")"
    else
      VAL_IMG=""
    fi
    if [[ -n "$VAL_IMG" && -d "$VAL_IMG" ]]; then
      LABELS_VAL="$(echo "$VAL_IMG" | sed 's#/images/#/labels/#')"
      case "$LABELS_VAL" in
        */val|*/val/) : ;;
        *) LABELS_VAL="${LABELS_VAL%/}/val" ;;
      esac
    fi
  fi
fi

[[ -n "$LABELS_VAL" && -d "$LABELS_VAL" ]] || {
  echo "Could not resolve labels/val. Provide -l /path/to/labels/val or add 'path:' to $DATA."; exit 1; }

# ---------- environment (robust & slightly faster IO) ----------
export PYTHONUNBUFFERED=1
export PYTHONIOENCODING=UTF-8
export PYTHONDONTWRITEBYTECODE=1
# ensure utils_event can be imported regardless of cwd
...
# quick once-per-run info
python - <<'PY' || true
import sys, platform
try:
    import ultralytics as u
    v = getattr(u, "__version__", "?")
except Exception:
    v = "n/a"
...
for D in "${DUMPS[@]}"; do
  ...
  LABDIR="$D/labels"
  [[ -f "$PJ" || -d "$LABDIR" ]] || continue

  tag="$(basename "$D")"  # e.g., iou0.50_c0.05_val
  ...
  # tiny, informative line
  sz="$(du -h "$PJ" 2>/dev/null | awk '{print $1}')"
  mt="$(date -r "$PJ" '+%F %T' 2>/dev/null || echo '?')"

  if [[ -s "$out_csv" ]]; then
    printf "[%d/%d] %-22s  %6s  %s  -> skip (exists)\n" "$i" "$TOTAL" "$tag" "${sz:--}" "$mt"
    continue
  fi
  printf "[%d/%d] %-22s  %6s  %s  -> running...\n" "$i" "$TOTAL" "$tag" "${sz:--}" "$mt"

  tmp="${out_csv}.tmp"

  # time the Python job: elapsed + max RSS (kB)
  if /usr/bin/time -f "    done in %E, maxRSS=%MkB" \
     python -u "$EVAL_PY" \
      --labels "$LABELS_VAL" \
      --pred_json "$PJ" \
      --out_csv "$tmp" \
...
python - <<PY > "$SUM_JSON" 2>/dev/null || true
...
[[ -s "$SUM_JSON" ]] && echo "Summary: $SUM_JSON" || true
exit $status
